/*
조이스틱으로 로봇팔(서보모터 4개)을 제어하고,
왼쪽 조이스틱 버튼을 눌러 현재 각도를 시리얼 모니터에 출력하는 프로그램

목적:
- 조이스틱 입력을 통해 로봇팔의 각 관절(bottom, arm1, arm2, grip)을 움직임
- 버튼 입력 시 현재 각도를 기록/출력하여 "간접 교시" 방식으로 동작을 저장하도록 함
*/
...
void setup() {
  ...
  pinMode(8, INPUT_PULLUP); // 위치 찾기용 버튼
...
void loop() {
  ...
  // 왼쪽 조이스틱 버튼(핀 8)이 눌렀을 때 현재 각도 출력
  if (digitalRead(8) == LOW){
    Serial.print(cnt);
    Serial.print("  Angles: ");
    Serial.print(preVal[0]); Serial.print(", ");
    Serial.print(preVal[1]); Serial.print(", ");
    Serial.print(preVal[2]); Serial.print(", ");
    Serial.println(preVal[3]);
    cnt += 1;
    delay(300);
  }
  delay(20);
